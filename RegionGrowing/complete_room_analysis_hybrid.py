#!/usr/bin/env python3
"""
Complete Room Analysis Pipeline - Hybrid Implementation

Hybrid approach combining:
- Phase 1: Octree (coarse outline, lower depth for speed)
- Phase 2: Point-based gap filling (only in boundary regions)
- Phase 3: RANSAC refinement (for accurate boundaries)

Executes all three steps:
1. Classification of walls, floors, and ceilings (octree-based region growing)
2. Segmentation of rooms using hybrid approach (Octree -> Point-based -> RANSAC)
3. Calculation of room areas and generation of 2D map

Usage:
    python complete_room_analysis_hybrid.py input.laz output_folder [options]
...
Output files in output_folder:
    ...
    - room_polygons.json: Room polygon coordinates
"""
...
# ============================================================================
# Point-Based Region Growing (for gap filling)
# ============================================================================
...
# ============================================================================
# RANSAC Plane Fitting (for boundary refinement)
# ============================================================================
...
# ============================================================================
# Hybrid Room Segmentation (Octree -> Point-based -> RANSAC)
# ============================================================================

class HybridRoomSegmenter:
    def __init__(self, points, classification, octree_max_depth=8, min_cell_size=0.01):
        self.points = points
    ...
    def _phase2_point_based_gap_filling(self, initial_labels, room_id_map, 
        ...
        print("Phase 2: Point-based gap filling in boundary regions (ceiling points only)...")
        ...
        print(f"Phase 2: Added {points_added} ceiling points through gap filling")
        return refined_labels
    
    def _phase3_ransac_refinement(self, labels, room_id_map, ransac_distance_threshold=0.05):
        print("Phase 3: RANSAC-based boundary refinement (ceiling points only)...")
        ...
        print(f"Phase 3: Refined {points_refined} ceiling points using RANSAC")
        return refined_labels
    ...
    def segment_rooms_hybrid(self, dilation_iterations=3, min_room_points=5000,
        ...
        print(f"Hybrid segmentation: Found {valid_rooms} valid rooms")
...
def step2_segment_rooms(input_file, output_file, octree_max_depth=8, min_cell_size=0.01,
                        ...
                        use_point_filling=True, use_ransac_refinement=True):
    print("\n" + "="*70)
    print("STEP 2: ROOM SEGMENTATION (Hybrid: Octree -> Point-based -> RANSAC)")
    print("="*70)
    ...
    print(f"Building hybrid segmenter (octree_max_depth={octree_max_depth}, min_cell_size={min_cell_size}m)...")
    segmenter = HybridRoomSegmenter(
...
def plot_map(grid, bounds, room_stats, output_image, cmap_name="tab20"):
    ...
    plt.title("Room segmentation map with surface areas (Hybrid: Octree -> Point-based -> RANSAC)")
    plt.xlabel("X (m)")
    plt.ylabel("Y (m)")
    plt.axis('equal')
...
def main():
    parser = argparse.ArgumentParser(
        description="Complete room analysis pipeline (Hybrid): classify, segment, and measure rooms",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
  python complete_room_analysis_hybrid.py input.laz output_folder
  python complete_room_analysis_hybrid.py input.laz output_folder --octree-max-depth 8 --use-ransac
...
Hybrid Approach:
  Phase 1: Octree (coarse outline, fast)
  Phase 2: Point-based (gap filling in boundaries)
  Phase 3: RANSAC (accurate boundary refinement)
        """
    )
    ...
    parser.add_argument("--octree-max-depth", type=int, default=8,
                       help="Octree maximum depth for Phase 1 (default: 8, lower=faster)")
    ...
    print("\n" + "="*70)
    print("COMPLETE ROOM ANALYSIS PIPELINE (HYBRID)")
    print("="*70)
    ...
    print(f"  Phase 2: Point-based gap filling ({'enabled' if args.use_point_filling else 'disabled'})")
    print(f"  Phase 3: RANSAC refinement ({'enabled' if args.use_ransac_refinement else 'disabled'})")
