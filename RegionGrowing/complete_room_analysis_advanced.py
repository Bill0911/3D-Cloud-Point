#!/usr/bin/env python3
"""
Complete Room Analysis Pipeline - Advanced Implementation

Advanced features for maximum accuracy:
- Hybrid segmentation (Octree -> Point-based -> RANSAC)
- Attachment detection (objects attached to walls/ceilings/floors)
...
Executes all steps:
1. Classification of walls, floors, and ceilings
2. Attachment detection and merging
3. Door frame detection
4. Room segmentation with boundary refinement
5. Object detection
6. Room type classification
7. Area calculation and map generation

Usage:
    python complete_room_analysis_advanced.py input.laz output_folder [options]

Output files in output_folder:
    - classified.las: Classified point cloud
    - segmented.las: Point cloud with room_id per room
    - room_map.png: 2D map with room labels, areas, and types
    - room_areas.csv: CSV with room areas and types
    - room_polygons.json: Room polygon coordinates with types
"""
...
def detect_attached_objects(points, classification, attachment_threshold=0.08, 
                           normal_parallel_threshold=0.85):
    print("\n" + "="*70)
    print("STEP 2: ATTACHMENT DETECTION")
    print("="*70)
    ...
    pcd_wall.points = o3d.utility.Vector3dVector(wall_points)
    kdtree_wall = o3d.geometry.KDTreeFlann(pcd_wall)
    
    pcd_ceiling = o3d.geometry.PointCloud()
    pcd_ceiling.points = o3d.utility.Vector3dVector(ceiling_points)
    kdtree_ceiling = o3d.geometry.KDTreeFlann(pcd_ceiling)
    
    pcd_floor = o3d.geometry.PointCloud()
    pcd_floor.points = o3d.utility.Vector3dVector(floor_points)
    kdtree_floor = o3d.geometry.KDTreeFlann(pcd_floor)
    ...
    for i, unclassified_idx in enumerate(unclassified_indices):
        ...
        point = points[unclassified_idx]
        normal = normals_all[unclassified_idx]
        ...
        if len(wall_points) > 0:
            [_, wall_idx, _] = kdtree_wall.search_knn_vector_3d(point, 1)
            if len(wall_idx) > 0:
                wall_point = wall_points[wall_idx[0]]
                distance = np.linalg.norm(point - wall_point)
                
                if distance < attachment_threshold:
                    wall_point_idx = np.where(wall_mask)[0][wall_idx[0]]
        ...
        if len(ceiling_points) > 0:
            [_, ceiling_idx, _] = kdtree_ceiling.search_knn_vector_3d(point, 1)
            if len(ceiling_idx) > 0:
                ceiling_point = ceiling_points[ceiling_idx[0]]
                distance = np.linalg.norm(point - ceiling_point)
                
                if distance < attachment_threshold:
                    ceiling_point_idx = np.where(ceiling_mask)[0][ceiling_idx[0]]
        ...
        if len(floor_points) > 0:
            [_, floor_idx, _] = kdtree_floor.search_knn_vector_3d(point, 1)
            if len(floor_idx) > 0:
                floor_point = floor_points[floor_idx[0]]
                distance = np.linalg.norm(point - floor_point)
                
                if distance < attachment_threshold:
                    floor_point_idx = np.where(floor_mask)[0][floor_idx[0]]
...
# ============================================================================
# Point-Based Region Growing & RANSAC
# ============================================================================
...
def ransac_plane_fit(points, max_iterations=1000, distance_threshold=0.05, min_inliers=3):
    if len(points) < 3:
...
def refine_segment_with_ransac(segment_points, distance_threshold=0.05):
    if len(segment_points) < 3:
        return segment_points
    
    normal, center, inliers = ransac_plane_fit(
        segment_points,
        max_iterations=500,
        distance_threshold=distance_threshold,
        min_inliers=3
...
# ============================================================================
# Hybrid Room Segmentation with Door Frame Boundaries
# ============================================================================

class AdvancedRoomSegmenter:
    def __init__(self, points, classification, door_frames, octree_max_depth=8, min_cell_size=0.01):
        self.points = points
    ...
    def _phase1_octree_coarse(self, dilation_iterations=3, min_room_points=5000):
        print("Phase 1: Octree-based coarse segmentation with door frame boundaries...")
    ...
    def _phase2_point_based_gap_filling(self, initial_labels, room_id_map, 
                                        normal_threshold=0.9, distance_threshold=0.1,
                                        gap_search_radius=0.3):
        print("Phase 2: Point-based gap filling in boundary regions...")
        
        unallocated_mask = initial_labels == 0
        unallocated_indices = np.where(unallocated_mask)[0]
        
        if len(unallocated_indices) == 0:
            print("Phase 2: No unallocated points to process")
        ...
        visited[allocated_indices] = True
        ...
        for room_id in room_id_map.values():
            ...
            boundary_points = []
            for idx in room_indices[:min(500, len(room_indices))]:
                [_, neighbor_indices, _] = self.kdtree.search_radius_vector_3d(
                    self.points[idx],
                    gap_search_radius
                )
                for neighbor_idx in neighbor_indices[1:]:
                    if neighbor_idx in unallocated_indices and not visited[neighbor_idx]:
            ...
            for seed_idx in boundary_points[:min(100, len(boundary_points))]:
                ...
                region_indices = region_growing_point_based(
                    self.points, self.normals, self.kdtree, seed_idx, visited,
                    normal_threshold=normal_threshold,
                    distance_threshold=distance_threshold,
                    min_region_size=50
                )
                ...
                if len(region_indices) > 0:
                    refined_labels[region_indices] = room_id
                    points_added += len(region_indices)
        
        print(f"Phase 2: Added {points_added} points through gap filling")
        return refined_labels
    
    def _phase3_ransac_refinement(self, labels, room_id_map, ransac_distance_threshold=0.05):
        print("Phase 3: RANSAC-based boundary refinement...")
        ...
        for room_id in room_id_map.values():
            ...
            room_points = self.points[room_indices]
        ...
        print(f"Phase 3: Refined {points_refined} points using RANSAC")
        return refined_labels
    ...
    def segment_rooms_advanced(self, dilation_iterations=3, min_room_points=5000,
        ...
        if use_point_filling:
            initial_labels = self._phase2_point_based_gap_filling(
        ...
        if use_ransac_refinement:
            initial_labels = self._phase3_ransac_refinement(
                initial_labels, room_id_map,
        ...
        print(f"Advanced segmentation: Found {valid_rooms} valid rooms")
...
def step2_segment_rooms_advanced(input_file, output_file, points, classification, door_frames,
                                ...
                                use_point_filling=True, use_ransac_refinement=True):
    print("\n" + "="*70)
    print("STEP 2: ADVANCED ROOM SEGMENTATION")
    print("="*70)
    
    print(f"Building advanced segmenter (octree_max_depth={octree_max_depth}, min_cell_size={min_cell_size}m)...")
...
def plot_map(grid, bounds, room_stats, room_classifications, output_image, cmap_name="tab20"):
    ...
    plt.title("Room Segmentation Map with Types and Areas\n(Advanced: Attachment Detection + Door Frames + Object Detection)")
    plt.xlabel("X (m)")
    plt.ylabel("Y (m)")
    plt.axis('equal')
...
def main():
    parser = argparse.ArgumentParser(
        description="Complete room analysis pipeline (Advanced): Maximum accuracy with all features",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Example:
  python complete_room_analysis_advanced.py input.laz output_folder
  python complete_room_analysis_advanced.py input.laz output_folder --octree-max-depth 8
...
Advanced Features:
  - Attachment detection (objects merged into walls/ceilings/floors)
  - Door frame detection (room boundaries)
  - Object detection (sinks, toilets, stoves, etc.)
  - Room type classification (bedroom, kitchen, bathroom, etc.)
        """
    ...
    print("\n" + "="*70)
    print("COMPLETE ROOM ANALYSIS PIPELINE (ADVANCED)")
    print("="*70)
    ...
    try:
        ...
        classification = detect_attached_objects(
