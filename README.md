...
* Currently experimenting to automate the process of finding the sweet spots. Instead of relying on the parameter report, this tool supports the client to identify sweet spots quicker. While the team aims to make the tools works universally (can deal with all kinds of las/laz file). The current challenge is that the tool only gives accurate sweet spots with only SOR las/laz files.
![Sweet_spot_detection Result](sweet_spot_graph.png)
